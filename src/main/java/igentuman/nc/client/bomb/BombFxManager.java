...
public class BombFxManager {
    ...
    private static void spawnFireball(ClientLevel level, ActiveBomb b) {
        ...
        for (int i = 0; i < fireCount; i++) {
            ...
            level.addAlwaysVisibleParticle(NcParticles.FIRE_VERTICAL1.get(), true, x, y, z, 0, vy, 0);
            if ((i & 1) == 0) {
    ...
    private static void spawnMushroom(ClientLevel level, ActiveBomb b) {
        ...
        for (int band = 0; band < stemBands; band++) {
            ...
            for (int i = 0; i < perBand; i++) {
                ...
                if (band < 2 && i % 4 == 0) {
                    level.addAlwaysVisibleParticle(NcParticles.FIRE_VERTICAL1.get(), true, x, y, z, 0, vy * 0.4, 0);
                }
                if (i % 5 == 0) {
        ...
        for (int i = 0; i < coreCount; i++) {
            ...
            double z = cz + r * Math.sin(a);
            double y = cy + h;
            level.addAlwaysVisibleParticle(NcParticles.EXPLOSION.get(), true, x, y, z, 0, 0.05, 0);
        }
        ...
        if (b.tickCounter >= b.capStart()) {
            ...
            for (int i = 0; i < domeCount; i++) {
                ...
                if ((i & 1) == 0) {
                    level.addAlwaysVisibleParticle(NcParticles.EXPLOSION_SEED.get(), true, x, y, z, vx * 0.5, 0.02, vz * 0.5);
                }
                if ((i & 3) == 0) {
                    level.addAlwaysVisibleParticle(NcParticles.FIRE_VERTICAL1.get(), true, x, y + 2, z, vx, 0.05, vz);
                }
            ...
            for (int i = 0; i < emitterCount; i++) {
                ...
                level.addAlwaysVisibleParticle(NcParticles.EXPLOSION.get(), true, x, y, z, 0, 0.02, 0);
            }
        }
    }
    ...
    private static void spawnGroundCloud(ClientLevel level, ActiveBomb b) {
        ...
        for (int i = 0; i < ringDensity; i++) {
            ...
            if ((i & 3) == 0) {
                level.addAlwaysVisibleParticle(NcParticles.SMOKE.get(), true, x, y, z, vx * 0.5, 0.01, vz * 0.5);
            }
            if ((i & 5) == 0) {
                level.addAlwaysVisibleParticle(NcParticles.FIRE_VERTICAL1.get(), true, x, y, z, vx * 1.2, 0, vz * 1.2);
            }
        }
    ...
    private static void playZoneSound(ActiveBomb b) {
        ...
        DeferredHolder<SoundEvent, SoundEvent> sound = pickSound(d);
        if (sound == null) return;
        mc.getSoundManager().play(SimpleSoundInstance.forUI(sound.get(), 1.0f, 1.0f));
    }

    private static DeferredHolder<SoundEvent, SoundEvent> pickSound(double d) {
