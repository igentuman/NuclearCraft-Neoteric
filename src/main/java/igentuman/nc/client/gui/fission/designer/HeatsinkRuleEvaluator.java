...
public class HeatsinkRuleEvaluator {
    ...
    public HeatsinkRuleEvaluator(DesignGrid grid,
        ...
        var ro = FissionReactorRegistration.FISSION_BLOCKS.get("fission_reactor_casing");
        this.casing = ro != null && ro.isPresent() ? ro.get() : null;
    }
    ...
    private boolean isModerator(Block block) {
        return FissionReactorRegistration.moderatorBlocks().contains(block);
    }
    ...
    private boolean validatePlacement(String[] condition, BlockPos pos) {
        ...
        if (isModerator(target) && !moderators.contains(pos)) {
            return false;
        }
    ...
    private boolean validateFuelCellAttachment(BlockPos p) {
        for (Direction dir : Direction.values()) {
            ...
            if (blockAt(np) instanceof FissionFuelCellBlock) {
                return true;
            }
        }
        return false;
