...
public class FusionCoolantEmiCategory extends BasicEmiRecipe {
    public static final EmiRecipeCategory CATEGORY = new EmiRecipeCategory(
            rl("fusion_coolant"),
            EmiStack.of(CATALYSTS.containsKey("fusion_coolant") ? 
                new ItemStack(FUSION_BLOCKS.get("fusion_core").get()) : 
                new ItemStack(BARRIER))
    );

    private final FusionCoreBE.FusionCoolantRecipe recipe;

    public FusionCoolantEmiCategory(FusionCoreBE.FusionCoolantRecipe recipe) {
        super(CATEGORY, recipe.getId(), 106, 35);
        this.recipe = recipe;
