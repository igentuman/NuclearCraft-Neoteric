...
public class AcceleratorCoolantCategoryWrapper<T extends LinearAcceleratorControllerBE.CoolantRecipe> implements IRecipeCategory<T> {
    ...
    public AcceleratorCoolantCategoryWrapper(IGuiHelper guiHelper, RecipeType<T> recipeType) {
        ...
        if(CATALYSTS.containsKey(getRecipeType().getUid().getPath())) {
            this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ACCELERATOR_BLOCKS.get("accelerator_port").get()));
        } else{
    ...
    public @NotNull List<Component> getTooltipStrings(@NotNull T recipe, @NotNull IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        ...
        if(mouseX > 34 && mouseX < 76 && mouseY > 6 && mouseY < 20) {
            lines.add(__("fusion_core.recipe.cooling_rate", (int)recipe.getCoolingRate()).withStyle(ChatFormatting.GOLD));
        }
        return lines;
    }
