...
public class MSRCategoryWrapper<T extends MSRControllerBE.Recipe> implements IRecipeCategory<T> {
    ...
    public MSRCategoryWrapper(IGuiHelper guiHelper, RecipeType<T> recipeType) {
        this.recipeType = recipeType;
        this.guiHelper = guiHelper;
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 98, 30);
        if(CATALYSTS.containsKey(getRecipeType().getUid().getPath())) {
            this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(FISSION_BLOCKS.get("msr_controller").get()));
        } else{
    ...
    public @NotNull List<Component> getTooltipStrings(T recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        ...
        if(mouseX > 29 && mouseX < 65 && mouseY > 8 && mouseY < 24) {
            ...
            lines.add(__("fission.recipe.criticality", (int)recipe.getCriticality()).withStyle(ChatFormatting.GOLD));
        }
        return lines;
    }
