...
public class KugelblitzChamberPonderScenes {
    ...
    public static void create(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world().setBlock(terminalPos, ModEntries.get("chamber_terminal").block().get().defaultBlockState().setValue(HORIZONTAL_FACING, Direction.EAST), false);
        scene.overlay().showText(60)
                .text("When all 6 lasers burst simultaneously, this will create a Blackhole inside the chamber.");
        scene.world().setBlock(new BlockPos(10, 10, 10), ModEntries.get("black_hole").block().get().defaultBlockState(), false);
        scene.idle(70);
    }
}
