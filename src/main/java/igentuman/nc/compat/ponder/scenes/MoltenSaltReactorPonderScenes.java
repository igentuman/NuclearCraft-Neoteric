...
public class MoltenSaltReactorPonderScenes {
    ...
    public static void create(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        BlockState glass = ModEntries.get("fission_reactor_glass").block().get().defaultBlockState();
        ...
        scene.addKeyframe();
        scene.world().setBlock(util.grid().at(3, 1, 0), ModEntries.get("msr_controller").block().get().defaultBlockState().setValue(HORIZONTAL_FACING, Direction.NORTH), true);
        ...
        scene.addKeyframe();
        BlockState port = ModEntries.get("msr_port").block().get().defaultBlockState();
        scene.world().setBlock(util.grid().at(0, 1, 2), port, true);
        scene.world().setBlock(util.grid().at(0, 1, 4), port, true);
        scene.world().setBlock(util.grid().at(6, 1, 2), port, true);
        scene.world().setBlock(util.grid().at(6, 1, 4), port, true);
        ...
        scene.idle(15);
        scene.world().setBlock(util.grid().at(3, 1, 0), ModEntries.get("msr_controller").block().get().defaultBlockState().setValue(HORIZONTAL_FACING, Direction.NORTH), false);
