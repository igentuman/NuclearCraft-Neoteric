...
public class TargetChamberPonderScenes {
    ...
    public static void create(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay().showOutline(PonderPalette.WHITE, new Object(), util.select().position(2, 2, 2), 30);
        ...
        scene.idle(45);
        scene.overlay().showControls(util.vector().topOf(-1, 2, 2), Pointing.LEFT, 30).withItem(new ItemStack(MULTITOOL.get())).rightClick();
        scene.idle(10);
        ...
        scene.overlay().showControls(util.vector().topOf(2, 3, 5), Pointing.LEFT, 30).withItem(new ItemStack(LEVER)).rightClick();
