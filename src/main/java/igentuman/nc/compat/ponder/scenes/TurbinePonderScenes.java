...
public class TurbinePonderScenes {
    ...
    public static void create(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world().setBlock(util.grid().at(4, 3, 0), ModEntries.get("turbine_controller").block().get().defaultBlockState().setValue(HORIZONTAL_FACING, Direction.NORTH), false);
        scene.world().setBlock(util.grid().at(0, 3, 3), ModEntries.get("turbine_bearing").block().get().defaultBlockState(), false);
        scene.world().setBlock(util.grid().at(8, 3, 3), ModEntries.get("turbine_bearing").block().get().defaultBlockState(), false);
        ...
        scene.idle(55);
        scene.addKeyframe();
        scene.world().setBlock(util.grid().at(4, 1, 0), ModEntries.get("turbine_port").block().get().defaultBlockState(), true);
        ...
        // Show Rotor Shaft
        for (int x = 1; x <= 7; x++) {
            scene.world().setBlock(util.grid().at(x, 3, 3), ModEntries.get("turbine_rotor_shaft").block().get().defaultBlockState().setValue(TurbineRotorBlock.FACING, Direction.EAST), true);
        ...
        for (int[] offset : coilOffsets) {
            scene.world().setBlock(util.grid().at(0, 3 + offset[0], 3 + offset[1]), ModEntries.get("turbine_magnesium_coil").block().get().defaultBlockState(), true);
