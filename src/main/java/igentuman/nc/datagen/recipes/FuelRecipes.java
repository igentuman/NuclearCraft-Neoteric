...
public class FuelRecipes extends NCRecipes {
            ...
            for(String subType: FuelManager.all().get(name).keySet()) {
                if(!name.matches("xenorium.*|quantite.*")) {
                    ...
                    if(name.equals("mixed")) {
                        ShapelessRecipeBuilder.shapeless(NC_FUEL.get(List.of("fuel", name, subType, "")).get(), 3)
                                .group(MODID+"_ingots")
                        ...
                        ShapelessRecipeBuilder.shapeless(NC_FUEL.get(List.of("fuel", name, subType, "ox")).get(), 3)
                                .group(MODID+"_ingots")
                                .requires(getIsotope("plutonium", String.valueOf(FuelManager.all().get(name).get(subType).getOxide().isotopes[0]), ""), 1)
                        ...
                        ShapelessRecipeBuilder.shapeless(NC_FUEL.get(List.of("fuel", name, subType, "ni")).get(), 3)
                                .group(MODID+"_ingots")
                                .requires(getIsotope("plutonium", String.valueOf(FuelManager.all().get(name).get(subType).getOxide().isotopes[0]), ""), 1)
                        ...
                        ShapelessRecipeBuilder.shapeless(NC_FUEL.get(List.of("fuel", name, subType, "za")).get(), 3)
                                .group(MODID+"_ingots")
                                .requires(getIsotope("plutonium", String.valueOf(FuelManager.all().get(name).get(subType).getOxide().isotopes[0]), ""), 1)
    ...
    private static void fuelPelletRecipe(Consumer<FinishedRecipe> consumer, String name, String subType, String type, int isotope1, int isotope2)
    {
        ...
        ShapelessRecipeBuilder.shapeless(NC_FUEL.get(List.of("fuel", name, subType, type)).get(), 3)
                .group(MODID+"_ingots")
