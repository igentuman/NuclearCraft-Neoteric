...
public class PsychoAnomalyEntity extends AnomalyEntity {
    ...
    protected void applyEffects() {
        ...
        int interval = Common.ANOMALY_CONFIG.PSYCHO_INTERVAL.get();
        if (!onInterval(interval)) {
            return;
        }
        double radius = Common.ANOMALY_CONFIG.PSYCHO_RADIUS.get();
        double r2 = radius * radius;
        int amplifier = Common.ANOMALY_CONFIG.PSYCHO_AMPLIFIER.get();
    ...
    private void tickVexSpawn() {
        ...
        int max = Common.ANOMALY_CONFIG.PSYCHO_VEX_MAX.get();
        if (max <= 0) {
            return;
        }
        double radius = Common.ANOMALY_CONFIG.PSYCHO_RADIUS.get();
    ...
    private int nextVexDelay() {
        int base = Math.max(1, Common.ANOMALY_CONFIG.PSYCHO_VEX_INTERVAL.get());
    ...
    private void spawnVex() {
        ...
        int lifetime = Common.ANOMALY_CONFIG.PSYCHO_VEX_LIFETIME.get();
        if (lifetime > 0) {
