...
public class FluidRenderProps implements IClientFluidTypeExtensions {
    ...
    public FluidRenderProps(String still, String flowing, int colorTint) {
        this.still = still.indexOf(':') > 0 ? neoforgeRl(still) : rl("block/material/fluid/" + still);
        this.flowing = flowing.indexOf(':') > 0 ? neoforgeRl(flowing) : rl("block/material/fluid/" + flowing);
