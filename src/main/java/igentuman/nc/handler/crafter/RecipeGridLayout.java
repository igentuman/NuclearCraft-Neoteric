...
/**
 * Maps a crafting recipe's ingredients onto a 3x3 grid (row-major, index {@code row * 3 + col}).
 * Shaped recipes keep their layout top-left aligned so the produced grid re-matches the same recipe;
 * anything else (shapeless and mod crafting recipes) is packed sequentially.
 */
