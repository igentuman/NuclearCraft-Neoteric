...
public class WorldRadiation implements IWorldRadiationCapability {
    ...
    public ConcurrentHashMap<Long, Long> chunkRadiation = new ConcurrentHashMap<>();
    public ConcurrentHashMap<Long, Long> updatedChunks = new ConcurrentHashMap<>();
    public ConcurrentHashMap<Long, Long> newChunks = new ConcurrentHashMap<>();
    public Level level;
    ...
    public int chunkRadiation(int chunkX, int chunkZ) {
        ...
        Long val = chunkRadiation.get(id);
        return val != null ? unpackX(val) : 0;
    }
    ...
    private int iterateNearbyChunks(int chunkX, int chunkZ) {
        ...
        for (int x = chunkX - radius; x <= chunkX + radius; x++) {
            for (int z = chunkZ - radius; z <= chunkZ + radius; z++) {
                ...
                long id = pack(x, z);
                Long nearby = chunkRadiation.get(id);
                int chunkRadiationValue = nearby != null ? unpackX(nearby) : 0;
    ...
    public int addRadiation(Level level, double radiation, int x, int z)
    {
        ...
        // Use single get() instead of containsKey()+get() to avoid TOCTOU race
        Long existing = chunkRadiation.get(id);
        if(existing != null) {
            int curRadiation = unpackX(existing);
