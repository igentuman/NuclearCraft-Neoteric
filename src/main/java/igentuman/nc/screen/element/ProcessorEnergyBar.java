...
public class ProcessorEnergyBar extends AbstractWidget {
    ...
    private final UniversalProcessorBE processorBE;

    public ProcessorEnergyBar(int x, int y, UniversalProcessorBE processorBE) {
        this(x, y, 12, 70, processorBE);
    }

    public ProcessorEnergyBar(int x, int y, int w, int h, UniversalProcessorBE processorBE) {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ILongEnergyStorage energy = processorBE.energyStorage;
        ...
        if (isHovered) {
            ...
            graphics.renderComponentTooltip(
                    ...
                    List.of(
                            Component.literal(TextUtils.formatEnergy(safeStored) + " / " + TextUtils.formatEnergy(safeMax)),
                            __("tooltip.nuclearcraft.energy_per_tick", TextUtils.formatEnergy(processorBE.getEnergyPerTick())),
                            __("tooltip.nuclearcraft.speed_multiplier", processorBE.speedMultiplier)),
                    mouseX, mouseY
            );
        }
    }
