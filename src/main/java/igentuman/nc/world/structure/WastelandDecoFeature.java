...
public class WastelandDecoFeature extends Feature<NoneFeatureConfiguration> {
    public static final List<ResourceLocation> structures = new ArrayList<>();
    public static final RegistryObject<Feature<NoneFeatureConfiguration>> WASTELAND_DECO_FEATURE = FEATURE_REGISTER.register(
            "wasteland_deco",
            () -> new WastelandDecoFeature(NoneFeatureConfiguration.CODEC)
    );

    public WastelandDecoFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    /**
     * Finds all wasteland deco structures available in the datapack
     * @param templateManager The structure template manager
     * @return A list of resource locations for wasteland deco structures
     */
    private List<ResourceLocation> findWastelandStructures(StructureTemplateManager templateManager) {
        ...
        if (!structures.isEmpty()) {
            debugLog("Wasteland deco structures pool size: " + structures.size());
        }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (availableStructures.isEmpty()) {
            debugLog("No wasteland deco structures found in datapack");
            return false;
        }
        ...
        Rotation randomRotation = rotations[context.random().nextInt(rotations.length)];

        Mirror randomMirror = context.random().nextBoolean()
                ? Mirror.NONE
                : (context.random().nextBoolean() ? Mirror.LEFT_RIGHT : Mirror.FRONT_BACK);
