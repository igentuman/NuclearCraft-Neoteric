...
// centered (aspect-corrected) coord -> screen UV
vec2 toUV(vec2 cc) {
    float a = aspect();
...
// Branching radial lightning: intensity for one bolt seeded by `seed`.
// Thin hot core + soft glow halo, wandering path from stacked noise octaves.
float bolt(float angle, float normDist, float seed, float t) {
    float slot = floor(t * 9.0 + seed * 17.0);
    float life = fract(t * 9.0 + seed * 17.0);
    // sharp strike: quick rise, flickering decay
    ...
    float r = normDist;
    // many zig-zags: low + mid + high frequency angular wander along the radius
    ...
    float coreLine = 1.0 - smoothstep(0.0, thickness, da);            // hot thin core
    float glow = (1.0 - smoothstep(0.0, thickness * 6.0, da)) * 0.4;  // soft halo
...
void main() {
    ...
    // Second (copy) pass just composites the swap target back onto main.
    if (FxPass < 0.5) {
    ...
    // Foreground objects in front of the anomaly hide the effect.
    ...
    // ===================== GRAVITATIONAL =====================
    if (AnomType == 0) {
        float mag = max(0.05, Radius.y); // < 1 from driver => stronger bulge
        // negative lens: magnify/bulge outward, strongest at the core, fading to the rim
        float bulge = pow(1.0 - clamp(normDist, 0.0, 1.0), 2.0);
        float rot = bulge * 1.4;
        vec2 cc = rotateVector(centeredCoord, rot);
        float strength = (1.0 / mag) - 1.0;
        float distortion = 1.0 / (1.0 + strength * bulge); // < 1 => sample inward => bulge
        vec2 dcc = cc * distortion;
        vec3 scene = sampleScene(toUV(dcc));
        // black gradient halo: darkest at the core, fading out to the rim
    ...
    // ===================== ELECTRIC =====================
        ...
        // blue glow halo + white-hot bolt cores + flickering centre
    ...
    // ===================== BURNING =====================
        ...
        // heat haze only above the core; horizontal shimmer growing with height
        ...
        // warm flickering glow, biased upward like flames licking up
    ...
    // ===================== PSYCHO =====================
        ...
        // NO geometric rotation. Psychedelic colour field: gentle liquid wave warp + hue
        // cycling + sweeping colour rays + concentric pulse rings + chromatic split. All motion
        // comes from moving colour/wave PHASES, never from rotating pixels.
        float fade = 1.0 - smoothstep(outerRadius * 0.55, outerRadius, distToCenter);

        // non-rotational liquid warp: two crossing sine waves + radial ripple
        vec2 warp = vec2(sin(centeredCoord.y * 22.0 + t * 2.3),
                         sin(centeredCoord.x * 22.0 - t * 2.0)) * 0.006 * fade;
        float ripple = sin(distToCenter * 60.0 - t * 5.0);
        vec2 cc = centeredCoord + warp + dir.xy * ripple * 0.004 * fade;

        // chromatic aberration split along the radius
        ...
        // sweeping colour rays (angular phase moves with time => looks like rotating rays, but
        // nothing is geometrically rotated) and outward-moving pulse rings
        float rays  = 0.5 + 0.5 * sin(angle * 6.0 + t * 1.5 + distToCenter * 8.0);
        float rings = 0.5 + 0.5 * sin(distToCenter * 28.0 - t * 3.0);
        // hue cycling over angle + distance + time
        ...
        // colour-inversion waves: outward-travelling ring bands flip the colour to its negative,
        // eased so it pulses between normal and inverted instead of switching hard. A second
        // slower band crosses it for a less regular, more unsettling beat.
